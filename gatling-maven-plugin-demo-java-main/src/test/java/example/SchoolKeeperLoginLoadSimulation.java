...
public class SchoolKeeperLoginLoadSimulation extends Simulation {
  ...
  private static final int HOLD_SECONDS = Integer.getInteger("holdSeconds", 60);

  private static final HttpProtocolBuilder HTTP_PROTOCOL =
      http.baseUrl(BASE_URL)
          .acceptHeader("application/json")
          .contentTypeHeader("application/json")
          .userAgentHeader("Gatling Login Load");
  ...
  private static final ScenarioBuilder LOGIN_SCENARIO =
      scenario("SchoolKeeper login load")
...

  {
    setUp(
        ...
        .maxDuration(Duration.ofSeconds(RAMP_SECONDS + HOLD_SECONDS + 10));
  }
}
